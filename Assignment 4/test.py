...
def test_docker():
    ...
    subprocess.run(["docker", "build", "-t", "flaskapp", "."])

    # Run the Docker container
    docker_process = subprocess.Popen(["docker", "run", "-d", "-p", "5000:5000", "flaskapp"])
    ...
    # Send a POST request to the Flask app
    data = {
        'text': "subject: ready get ? hello ! viagra # 1 med struggle mens ' erectile dysfunction . like one jokes sais , strong enough man , made woman ; - ) orderinq viaqra oniine convinient , fast secure way ! miiiions peopie daiiy save privacy money order . . .",
        'threshold': 0.5
    }

    # Send a POST request to the Flask app
    response = requests.post(URL, json=data)
    # response = requests.get(URL,json="Hello")
    ...
    assert('propensity', response_json)
